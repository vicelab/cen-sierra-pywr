...
# initialize model
model.timestepper.delta = 30
model.timestepper.end = model.timestepper.start + timedelta(30 * 12)
model.setup()
delta = timedelta(30)
for i in tqdm(range(12*4), ncols=80):
    model.run()
    model.timestepper.start += delta
    model.timestepper.end += delta
