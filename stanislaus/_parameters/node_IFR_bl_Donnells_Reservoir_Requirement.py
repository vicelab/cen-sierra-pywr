...
class node_IFR_bl_Donnells_Reservoir_Requirement(WaterLPParameter):
    ...
    def _value(self, timestep, scenario_index):
        ...
        data_ifr = self.read_csv(path+fName,usecols=[0,1,2,3,4,5,6], index_col=None, header=0, names=['start_date','end_date','1','2','3','4','5'], parse_dates=[0,1])
        # Critically Dry: 1,Dry: 2,Normal-Dry: 3,Normal-Wet: 4,Wet: 5
        #Calculate regular IFR
        ifr_val = (data_ifr[(data_ifr['start_date'] <= dt) & (data_ifr['end_date'] >= dt)][str(WYT)])/35.314666
        # Calculate supp IFR
        ...
        if timestep.month == 10 and timestep.day == 1:
            peak_inflow_Donnells = self.read_csv('Scenarios/Livneh/runoff/peak_runoff_DonnellsRes_MarToMay_cms.csv',
                                             usecols=[0, 1], index_col=[0], parse_dates=[1], squeeze=True)  # cms
        ...
        if diff_day > 0 and diff_day <=91:
            ifr_supp = (data_supp[(data_supp['Day_st'] <= diff_day) & (diff_day < data_supp['Day_end'])][str(WYT)])/35.314666
