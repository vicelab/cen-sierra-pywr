...
class IFR_bl_Hetch_Hetchy_Reservoir_Water_Year_Type(WaterLPParameter):
    ...
    def _value(self, timestep, scenario_index):
        ...
        if timestep.month >= 10:
            start_year = timestep.year
        else:
            start_year = timestep.year - 1
        ...
        # iniitial IFR
        if timestep.datetime == self.dates[0]:
            WYT = 2 # default
        
        # These are monthly values, so only calculate values in the first time step of each month
        elif timestep.day > 10 or timestep.month in [9,10,11,12]:
            WYT = self.get("IFR bl Hetch Hetchy Reservoir/Water Year Type", offset=-1, **kwargs)
        
        # Jan-June:
        elif timestep.month in [1,2,3,4,5,6]:
            cumulative_precip = self.get("node/HETCH_HETCHY_RES/Precipitation", start=start, agg='sum')
        ...
        # July-Aug:
            ...
            average_cfs = self.get("node/HETCH_HETCHY_WATERSHED/Runoff", start=start, agg='mean')
