...
def linearize_prices(steps, periods, dst):
    outdir = dst
    ...
    for step in steps:
                    ...
                    try:
                        ...
                        if step == 'monthly':
                            ...
                            if isleap(year) and date.month == 12 and date.day == 31:
                                ...
                                data_day = date.day
                            _s = s[
                                (s.index.year == data_year) & (s.index.month == date.month) & (s.index.day == data_day)]
        ...
        # save prices for the time step
        all_prices_path = os.path.join(outdir, 'piecewise_prices_DpMWh_{}.csv'.format(step))
        all_prices.to_csv(all_prices_path)

        # save blocks for the time step
        all_blocks_path = os.path.join(outdir, 'piecewise_blocks_{}.csv'.format(step))
        all_blocks.to_csv(all_blocks_path)
...
if __name__ == '__main__':
    ...
    dst = './output'
    linearize_prices(steps, periods, dst)
