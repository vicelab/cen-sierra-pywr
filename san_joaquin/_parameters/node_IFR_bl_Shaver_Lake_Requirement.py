...
class node_IFR_bl_Shaver_Lake_Requirement(WaterLPParameter):
    ...
    def _value(self, timestep, scenario_index):
        ...
        management = 'BAU'
        path = "Management/{mgt}/IFRs/IFRblShaverLake.csv".format(mgt=management)
        data = self.read_csv(path, usecols=[0, 1, 2, 3, 4, 5], index_col=0, header=None,
                             names=['week', '1', '2', '3', '4', '5'], parse_dates=False)
        WYT = self.model.parameters['WYT_SJValley'].value(timestep, scenario_index)
        week = min(timestep.datetime.weekofyear, 52)
        ifr = data[str(WYT)][week] * 0.028317  # cfs to cms
